...
test('notes CRUD and targeting by conversation or phone number', () => {
  const { tenantId, db, raw, cleanup } = freshDb('notes');
  try {
    const convId = seedConversation(raw, { tenantId, phone: '+12015550199', name: 'Test Prospect' });

    // Test adding note for target with valid conversation ID
    const note1 = db.addNoteForTarget(tenantId, {
    ...
    const notesByConv = db.getNotesForTarget(tenantId, { conversationId: convId, phoneNumber: '+12015550199' });
    assert.strictEqual(notesByConv.length, 1);
    assert.strictEqual(notesByConv[0].id, note1.id);

    // Query notes by phone number only
    const notesByPhone = db.getNotesForTarget(tenantId, { conversationId: 0, phoneNumber: '+12015550199' });
    assert.strictEqual(notesByPhone.length, 1);
    assert.strictEqual(notesByPhone[0].id, note1.id);

    // Delete note
    const deleteRes = db.deleteNote(tenantId, note1.id);
    assert.strictEqual(deleteRes.changes, 1);

    const notesAfterDelete = db.getNotesForTarget(tenantId, { conversationId: convId, phoneNumber: '+12015550199' });
