...
function freshDb(label) {
  ...
  // Every test runs inside a real tenant. The scoped API takes tenant_id as a
  // required first argument and throws without one, so there is no such thing
  // as an unscoped call to test against - a default tenant is the baseline.
  const tenant = db.createTenant('Test Tenant', 'test-tenant');
  // A tenant with no numbers cannot send: DID ownership is what authorises
  // a sender, so the fixture tenant owns one.
  db.assignDidToTenant(tenant.id, '5555550100');
  ...
  return {
    ...
    tenantId: tenant.id,
    tenant,
    /** A second tenant, for tests that need to prove isolation. */
    addTenant(name) {
...
function seedConversation(raw, { tenantId, phone, name = null, stage = 'Stage 1', createdAt = null, ...rest }) {
  if (!tenantId) throw new Error('seedConversation requires tenantId');
  const columns = ['tenant_id', 'phone_number', 'name', 'stage'];
  const values = [tenantId, phone, name, stage];
...
function seedMessage(raw, { tenantId, conversationId, direction, body = '', status = null, createdAt = null, deliveredAt = null }) {
  if (!tenantId) throw new Error('seedMessage requires tenantId');
  const finalStatus = status || (direction === 'inbound' ? 'received' : 'sent');
  const columns = ['tenant_id', 'conversation_id', 'direction', 'from_number', 'to_number', 'body', 'status'];
  const values = [tenantId, conversationId, direction, 'from', 'to', body, finalStatus];
  if (createdAt) { columns.push('created_at'); values.push(createdAt); }
