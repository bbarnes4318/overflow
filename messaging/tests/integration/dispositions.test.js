...
function newConversation(name) {
  seq++;
  return db.getOrCreateConversation(tenantId, `+1555222${String(seq).padStart(4, '0')}`, name).id;
}
...
test('every valid disposition saves', () => {
  db.VALID_DISPOSITIONS.forEach(disposition => {
    ...
    const updated = db.setConversationDisposition(tenantId, 
...
test('an invalid disposition is rejected', () => {
  ...
  assert.throws(() => db.setConversationDisposition(tenantId, id, 'banana'), /Invalid disposition/);
  assert.throws(() => db.setConversationDisposition(tenantId, id, 'APPOINTMENT'), /Invalid disposition/);
});

test('appointment requires a datetime', () => {
  const id = newConversation('NoDate');
  assert.throws(() => db.setConversationDisposition(tenantId, id, 'appointment', null),
    /date and time is required/);
  assert.throws(() => db.setConversationDisposition(tenantId, id, 'appointment', ''),
    /date and time is required/);
});

test('follow_up requires a datetime', () => {
  const id = newConversation('NoDate2');
  assert.throws(() => db.setConversationDisposition(tenantId, id, 'follow_up', null),
    /date and time is required/);
});
...
test('non-scheduled dispositions never keep a schedule', () => {
  ...
  const updated = db.setConversationDisposition(tenantId, id, 'no', utcStamp(60 * 24));
...
test('clearing a disposition resets the fields', () => {
  ...
  db.setConversationDisposition(tenantId, id, 'appointment', utcStamp(60 * 24), 'bring pricing');
  const cleared = db.setConversationDisposition(tenantId, id, null);
...
test('a missing conversation returns undefined rather than throwing', () => {
  assert.strictEqual(db.setConversationDisposition(tenantId, 999999, 'no'), undefined);
});

test('rescheduling clears the fired reminder state', () => {
  const id = newConversation('Rescheduled');
  const first = utcStamp(60 * 24);
  db.setConversationDisposition(tenantId, id, 'appointment', first);

  raw.prepare("INSERT INTO reminder_state (tenant_id, conversation_id, scheduled_at, tier) VALUES (?, ?, ?, 'due_soon')")
    .run(tenantId, id, first);
  assert.strictEqual(
...
test('notes are stored and editable', () => {
  ...
  db.setConversationDisposition(tenantId, id, 'follow_up', utcStamp(60 * 24), 'wants quarterly pricing');
  assert.strictEqual(
    raw.prepare('SELECT disposition_note FROM conversations WHERE id = ?').get(id).disposition_note,
    'wants quarterly pricing');

  db.setConversationDisposition(tenantId, id, 'follow_up', utcStamp(60 * 24), 'changed: wants annual');
  assert.strictEqual(
    raw.prepare('SELECT disposition_note FROM conversations WHERE id = ?').get(id).disposition_note,
...
test('scheduled times are stored as UTC', () => {
  ...
  db.setConversationDisposition(tenantId, id, 'appointment', utc);
