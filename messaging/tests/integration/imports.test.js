...
test('a suppressed contact is NOT reported as imported', () => {
  ...
  db.bulkImportLeads(tenantId, [{ phone_number: '+15553330030', name: 'Gone' }], null, '5555550100');
  const id = raw.prepare('SELECT id FROM conversations WHERE phone_number = ?').get('+15553330030').id;
  db.recordOptOut(tenantId, id, { source: 'manual', text: 'STOP', actor: 'test' });
...
test('skip reasons are broken out by category', () => {
  const make = (phone, mutate) => {
    db.bulkImportLeads(tenantId, [{ phone_number: phone, name: 'x' }], null, '5555550100');
    const id = raw.prepare('SELECT id FROM conversations WHERE phone_number = ?').get(phone).id;
    mutate(id);
    return id;
  };
  make('+15553330040', id => db.recordOptOut(tenantId, id, { source: 'manual', actor: 't' }));
  make('+15553330041', id => db.recordWrongNumber(tenantId, id, { source: 'manual', actor: 't' }));
  make('+15553330042', id => db.setConversationDisposition(tenantId, id, 'unqualified'));
...
test('the structured skipped list names each blocked contact', () => {
  const result = db.bulkImportLeads(tenantId, [{ phone_number: '+15553330040' }], 'Hi', '5555550100');
...
test('an import without a template creates contacts but queues nothing', () => {
  const result = db.bulkImportLeads(tenantId, [{ phone_number: '+15553330050', name: 'Quiet' }], null, '5555550100');
  assert.strictEqual(result.new_contacts, 1);
...
test('bulk send reports the same structured skip data', () => {
  ...
  const result = db.sendBulkMessages(tenantId, [optedOut, fine], 'Hello', '5555550100');
